...
public class ExpiringStateExercise extends ExerciseBase {
	...
	public static class EnrichmentFunction extends KeyedCoProcessFunction<Long, TaxiRide, TaxiFare, Tuple2<TaxiRide, TaxiFare>> {
		...
		public void open(Configuration config) throws Exception {
			ValueStateDescriptor<TaxiRide> rideVSD = new ValueStateDescriptor<>("saved ride", TaxiRide.class);
			ValueStateDescriptor<TaxiFare> fareVSD = new ValueStateDescriptor<>("saved fare", TaxiFare.class);
			rideState = getRuntimeContext().getState(rideVSD);
			fareState = getRuntimeContext().getState(fareVSD);
		}
