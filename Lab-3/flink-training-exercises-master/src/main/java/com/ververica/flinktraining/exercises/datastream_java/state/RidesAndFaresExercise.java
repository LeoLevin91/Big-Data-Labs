...
public class RidesAndFaresExercise extends ExerciseBase {
	...
	public static class EnrichmentFunction extends RichCoFlatMapFunction<TaxiRide, TaxiFare, Tuple2<TaxiRide, TaxiFare>> {
		...
		private ValueState<TaxiRide> rideSt;
		private ValueState<TaxiFare> fareSt;
		@Override
		public void open(Configuration config) throws Exception {
			ValueStateDescriptor<TaxiRide> saveRide = new ValueStateDescriptor<>("saved ride", TaxiRide.class);
			ValueStateDescriptor<TaxiFare> saveFire = new ValueStateDescriptor<>("saved fare", TaxiFare.class);

			rideSt = getRuntimeContext().getState(saveRide);
			fareSt = getRuntimeContext().getState(saveFire);
		}

		@Override
		public void flatMap1(TaxiRide ride, Collector<Tuple2<TaxiRide, TaxiFare>> out) throws Exception {

			TaxiFare fare = fareSt.value();
			if (fare != null) {
				fareSt.clear();
